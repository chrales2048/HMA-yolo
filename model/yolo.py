...
from models.extra_modules.block import *
from models.extra_modules.up import *
from models.extra_modules.backbone.backbone import *
...
class DualDDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        Conv_type = 'Conv'      # Conv, RepConvN, DWConv, DWConvTranspose2d, DySnakeConv, GhostConv, LightConv

        c2, c3 = make_divisible(max((ch[0] // 4, self.reg_max * 4, 16)), 4), max((ch[0], min((self.nc * 2, 128))))  # channels
        c4, c5 = make_divisible(max((ch[self.nl] // 4, self.reg_max * 4, 16)), 4), max((ch[self.nl], min((self.nc * 2, 128))))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(eval(Conv_type)(x, c2, 3), Conv(c2, c2, 3, g=4), nn.Conv2d(c2, 4 * self.reg_max, 1, groups=4)) for x in ch[:self.nl])
        self.cv3 = nn.ModuleList(
            nn.Sequential(eval(Conv_type)(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch[:self.nl])
        self.cv4 = nn.ModuleList(
            nn.Sequential(eval(Conv_type)(x, c4, 3), Conv(c4, c4, 3, g=4), nn.Conv2d(c4, 4 * self.reg_max, 1, groups=4)) for x in ch[self.nl:])
        self.cv5 = nn.ModuleList(
            nn.Sequential(eval(Conv_type)(x, c5, 3), Conv(c5, c5, 3), nn.Conv2d(c5, self.nc, 1)) for x in ch[self.nl:])
        self.dfl = DFL(self.reg_max)
        self.dfl2 = DFL(self.reg_max)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # TODO: channel, gw, gd
            ...
            c2 = ch[f] // args[0] ** 2
            # ----------------------------------------------------------
        elif m in {BAM, CBAM, ECA, SimAM, CoordAtt, GAM_Attention, SKAttention, SE, LSKblock, A2, ACmix,
                   FocalModulation, ScConv, EfficientAttention, SCSA, FRFN}:
            c1 = ch[f]
            c2 = ch[f]
            args = [c1, *args]
        elif m is Partial_conv3:
            c1 = ch[f]
            c2 = c1
            args = [c1, *args]
        elif m in {AIFI, CARAFE, DySample, HWD, Conformer, GhostConformer, gnconv}:
            c1 = ch[f]
            c2 = ch[f]
            args = [c1, *args]
        elif m in {SPDConv, HWD_ADown, SPD_ADown}:
            c1 = ch[f]
            c2 = args[0]
            args = [c1, *args]
        elif m in {Ghost_SDI, SDI}:
            args = [[ch[fi] for fi in f]]
        elif m in {KANConv2DLayer}:
            args = [ch[f], args[0], args[1], args[2]]
            ...
            c2 = args[0]
        elif m in {cloformer_xxs, cloformer_global, cloformer_xs, cloformer_s, cloformer_no_global}:
            c2 = args[0]
        elif m in {MobileNetV4, ShuffleNetV1, ShuffleNetV2, EfficientNetV1, EfficientNetV2, SqueezeNet}:
            c2 = d['out'][0][0]
            ...
            c2 = c1[0]
            args = [c1]
        elif m in { C2f_MLKA_Ablation,
                   C3_MLKA_Ablatio,  AFDM, }:
            c1 = ch[f]
            c2 = args[0]
            args = [c1, *args]
        elif m in {
                   GhostInceptionV2c, GhostInceptionV2a, GhostInceptionV2b,
                   }:
            c1 = ch[f]
            c2 = args[0]
            args = [c1, *args]
        ...
        elif m in {CD_LSK, SCAM, SMFA}:
            c2 = c1 = ch[f]
            args = [c1, *args]

        elif m in {SKFusion, CGAFusion}:
            c2 = c1 = ch[f[0]]
            args = [c1, *args]
        ...
        elif m is RepVGGBlocks:
            c2 = ch[f]
            args = [ch[f], *args]
        elif m in {PagFM, PagFM_cdsimAM}:
            c1 = [ch[f[0]], ch[f[1]]]
            c2 = c1[0]
            args = [c1, *args]
        # ----------------------------------------------------------
